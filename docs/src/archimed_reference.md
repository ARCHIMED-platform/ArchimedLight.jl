...
The Julia implementation keeps the same user-facing logic but relies on the generic infrastructure provided by `PlantGeom` and `PlantMeteo`. The function `read_light_config` normalizes the YAML configuration into a `LightConfig` structure, and `read_scene` uses `PlantGeom.read_ops` and mesh-merging utilities to build a single scene graph with merged geometry and node-to-face mappings. The result is stored in `SceneGeometry`, which contains the merged mesh, the face-to-node map, per-node area and barycenter, and metadata such as group, type, item id, and component id. In other words, Java keeps a richer object tree alive during the computation, whereas Julia resolves more of the scene into dense arrays and dictionaries ahead of time.
...
The Julia implementation reproduces this logic in `build_scattering_transfer_graph` and `compute_scattering`. The current port supports the same broad semantics: waveband-specific scattering coefficients, hit-count normalization, exclusion of the direct sun sector from certain scattering paths when `all_in_turtle=false`, virtual-sensor transparency, and iterative stopping based on the relative amount of remaining scattered energy. The implementation is written in a more data-oriented style than Java, but conceptually it is the same model.
...
The easiest way to compare both implementations is to read them as two versions of the same pipeline. In Java, the orchestration class is `MirProcess`. It loads parameters and the scene, constructs the turtle, computes radiative fluxes, runs directional projections, computes first-order interception, optionally computes light-source contributions, optionally runs MUSC scattering, and finally writes outputs. In Julia, the equivalent high-level flow is exposed more explicitly: `read_light_config`, `read_scene`, `read_meteo`, `compute_sky`, `build_turtle`, `compute_directional_fluxes`, `compute_first_order`, `build_scattering_transfer_graph`, `compute_scattering`, `integrate_light`, and then the output writers. The steps are intentionally more composable, but the physics pipeline is the same.
...
At the time of writing, the largest remaining discrepancies are concentrated in exact border and toric rasterization parity. A few wrapped pixels near scene boundaries are still assigned differently in Julia and Java, and that is enough to create component-level differences in intercepted radiation for the affected fixtures. These errors are localized rather than systemic, which is why scene totals can already look good while some component rows still differ.

The current parity status is therefore mixed but encouraging. Several comparison fixtures already match at scene level, and many also match at component level. The remaining hard failures are limited to a small set of fixtures, with mismatch counts concentrated in `test-compare-cafeier1`, `test-compare-cafeier4`, and `test-compare-two_coffee`. There is also a small residual summary mismatch in `test-compare-simpleplant`, and one guard-profile threshold miss that is numerically tiny but still real. In practical terms, this means the Julia implementation is already a faithful reimplementation of the ARCHIMED light pipeline in structure and in most behaviors, but not yet a strict drop-in numerical equivalent for every frozen Java fixture.
